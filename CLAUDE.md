...
7. `convert.py` — `convert_file()` shells out to `HandBrakeCLI -i <src> -o <dest> --preset <preset> --all-subtitles`. `--all-subtitles` is not optional: the presets default to a forced-subtitle search, and a disc carrying only a normal full subtitle track has nothing flagged forced, so every conversion before this silently discarded its subtitles. `probe_height()` scans the source with HandBrake (not ffprobe — the pipeline keeps its single external dependency) and `select_preset()` picks `HANDBRAKE_PRESET` for SD sources (≤576 lines) or `HANDBRAKE_PRESET_HD` above that; an unreadable height falls back to the SD preset, since guessing HD on an SD source costs quality while the reverse only costs time. It also deletes a partial output file if the process exits non-zero *or* if the run is interrupted (Ctrl-C reaches HandBrake too, so it dies mid-write). `is_complete_output()` is the second half of that defense: it judges an *existing* destination by size against its source (`MIN_OUTPUT_SIZE_RATIO`, 10% — far below any plausible encode ratio, so it catches near-empty truncation without second-guessing compression). The cleanup handler only protects runs that die where Python can catch it; the size check repairs whatever slipped through, including stubs left by older versions of the code.
